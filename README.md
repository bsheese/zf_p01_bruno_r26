...
A description of the method and some preliminary findings were presented in a poster session in 2014: 
...
under `data/` is derived and regenerated by the scripts. Because the raw and
intermediate files are large, they may not be included directly in the Git
repository; an archived copy is available at **`<add archive / Zenodo DOI here>`**
or on request from the contact below.

## Citing

This is unpublished, poster-stage work. If you use the code or data, please cite
the repository (and its archived release, once minted):

> Sheese, B. *Short-term reliability of the zebrafish novel-tank test.* GitHub
> repository, `<add archive / Zenodo DOI here>`.
