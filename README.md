...
- **Strain is confounded with batch.** The dataset has two strains (5G, AB) but
  ...
  (date, handling, per-batch calibration). **No strain comparison is made in this
  reanalysis.** Strain/multivariate/clustering analyses are exploratory only and kept
...
```
src/            Analysis (reliability) pipeline — run in order, 01 → 12
supplemental/   Exploratory strain / multivariate / clustering work (strain
                confounded with batch). See supplemental/README.md
data/
...
output/         Results tables, text, and figures
  supplemental/ Outputs of the exploratory scripts
lit/            references.bib (cited by the manuscript) + a pointer note.
                The literature PDFs, text conversions (was lit/converted/), and
                Obsidian notes (was vault/) now live in the shared ../priorlit
                collection — see lit/README.md.
```
...
Run the analysis pipeline from the repo root, in order:
